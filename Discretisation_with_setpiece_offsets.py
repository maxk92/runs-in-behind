"""
Discrete_efforts_Ilana.py — VERSION WITH SETPIECE OFFSETS
-----------------------------------------------------------
Modified version that excludes movements whose START and/or END falls within
a setpiece offset window.

A movement is dropped if:
  - its start_frame falls inside [setpiece_frame, setpiece_frame + offset_frames), OR
  - its end_frame   falls inside [setpiece_frame, setpiece_frame + offset_frames).

Movements that merely pass through a blackout window mid-course (neither
starting nor ending in it) are kept.

OFFSETS (from EventTime):
- ThrowIn      : +2 seconds (+50 frames at 25 fps)
- FreeKick     : +3 seconds (+75 frames)
- GoalKick     : +3 seconds (+75 frames)
- KickOff      : +4 seconds (+100 frames)
- CornerKick   : +4 seconds (+100 frames)
...
# SETPIECE OFFSETS (in seconds)
SETPIECE_OFFSETS = {
    'ThrowIn':    2.0,  # 2 seconds = 50 frames at 25 fps
    'FreeKick':   3.0,  # 3 seconds = 75 frames
    'GoalKick':   3.0,  # 3 seconds = 75 frames
    'KickOff':    6.0,  # 6 seconds = 150 frames
    'CornerKick': 4.0,  # 4 seconds = 100 frames
}
...
def get_second_half_start_utc(path_events, path_info):
    """
    Return the UTC timestamp that anchors secondHalf tracking frame 0.

    WHY THIS IS NEEDED
    ------------------
    DFL secondHalf tracking starts at the H2 kickoff (frame 0 = kickoff),
    NOT at the H1 final whistle.  Using TotalTimeFirstHalf (end of H1) as
    the H2 anchor therefore shifts every H2 event_frame by the break
    duration (~18 min = ~27 500 frames), which:
      * puts H2 blackout windows ~18 min too late  -> setpieces after goals missed
      * extends the first-KickOff blackout over the first ~18 min of H2 play
      * makes h2_kickoff ~27 500 instead of 0, shifting all H2 timecodes ~18 min

    FIX
    ---
    Use the EventTime of the KickOff that carries GameSection="secondHalf"
    as the true H2 anchor.  TotalTimeFirstHalf is still used as a coarse
    H1-end estimate only to find that event (bootstrap).
    ...
    Returns
    -------
    pd.Timestamp (UTC) -- EventTime of the H2 kickoff
    """
    ...
    # Coarse H1-end estimate (used only to bootstrap the H2 event search)
    ...
    if other_info is not None and 'TotalTimeFirstHalf' in other_info.attrib:
        ...
        h1_end_utc    = kickoff_utc + pd.Timedelta(milliseconds=first_half_ms)
    else:
        h1_end_utc    = kickoff_utc + pd.Timedelta(minutes=50)

    ev_root = ET.parse(path_events).getroot()

    # Strategy 1: look for KickOff with explicit GameSection="secondHalf"
    for event in ev_root.findall('Event'):
        event_time_str = event.attrib.get('EventTime')
        if event_time_str is None:
            continue
        for child in event:
            if child.tag == 'KickOff' and child.attrib.get('GameSection') == 'secondHalf':
                h2_utc = pd.Timestamp(event_time_str).tz_convert('UTC')
                print(f"  [H2 anchor] KickOff GameSection=secondHalf found at {h2_utc} "
                      f"(break = {(h2_utc - h1_end_utc).total_seconds()/60:.1f} min)")
                return h2_utc

    # Strategy 2: first KickOff that occurs strictly after h1_end_utc
    for event in ev_root.findall('Event'):
        event_time_str = event.attrib.get('EventTime')
        if event_time_str is None:
            continue
        event_utc = pd.Timestamp(event_time_str).tz_convert('UTC')
        if event_utc <= h1_end_utc:
            continue
        for child in event:
            if child.tag == 'KickOff':
                print(f"  [H2 anchor] first KickOff after H1 end at {event_utc} (fallback)")
                return event_utc

    # Strategy 3: last-resort fallback
    print("  [H2 anchor][WARN] H2 KickOff not found -- falling back to H1-end estimate")
    return h1_end_utc

# ============================================================================
# Ball contact event parsing WITH SETPIECE OFFSETS
# ============================================================================

def parse_ball_contact_events(path_events, path_info, framerate=25, second_half_start_utc=None):
    """
    Parse ball-contact events from DFL events XML.
    ...
    # ── 2. Second-half anchor (pre-computed by process_match to avoid re-parsing)
    if second_half_start_utc is None:
        second_half_start_utc = get_second_half_start_utc(path_events, path_info)
...
def tag_ball_touches(df_movements, path_events, path_info,
                     framerate=25, second_half_start_utc=None):
    """
    ...
    """
    contacts = parse_ball_contact_events(path_events, path_info, framerate=framerate, second_half_start_utc=second_half_start_utc)
...
# ============================================================================
# Setpiece blackout window builder
# ============================================================================

def build_setpiece_blackout_frames(path_events, path_info, framerate=25, second_half_start_utc=None):
    """
    Build per-half sets of frames that fall within a setpiece offset window.

    For each setpiece event, all frames in [event_frame, event_frame + offset_frames)
    are marked as "blackout". Any movement whose start_frame OR end_frame falls
    inside a blackout window is excluded from the output. Movements that only
    cross a blackout window mid-course are intentionally kept.
    ...
    IMPORTANT — KickOff frame-0 fix
    --------------------------------
    The DFL matchinformation KickoffTime marks tracking frame 0, but the KickOff
    EventTime in the events XML is when the ball is *actually* kicked (typically
    3–10 s later).  Without correction, frames [0, event_frame) would be outside
    any blackout, letting pre-kick movements like "0:02" through.
    Fix: for the *first* KickOff in each half, the blackout window is extended
    backward to frame 0, i.e. range(0, event_frame + offset_frames).
    Subsequent KickOffs (after goals) use the standard range.
    ...
    Returns
    -------
    dict { 'firstHalf': set of int, 'secondHalf': set of int }
    """
    info_root   = ET.parse(path_info).getroot()
    kickoff_str = info_root.find('.//General').attrib['KickoffTime']
    kickoff_utc = pd.Timestamp(kickoff_str).tz_convert('UTC')

    # Use H2 KickOff EventTime as anchor (pre-computed by process_match to avoid re-parsing)
    if second_half_start_utc is None:
        second_half_start_utc = get_second_half_start_utc(path_events, path_info)
    ...
    # ── Track the first KickOff seen per half ─────────────────────────────────
    # BUG FIX: KickoffTime (matchinformation) = tracking frame 0, but the
    # KickOff EventTime is when the ball is *actually* kicked (typically 3–10 s
    # later). Without this fix, frames [0, event_frame) are not covered by any
    # blackout, letting pre-kick movements slip through.
    # Solution: for the first KickOff of each half, extend the blackout back to
    # frame 0. Subsequent KickOffs (after goals) use the standard window.
    ...
    for event in ev_root.findall('Event'):
        ...
        for child in event:
            ...
            # ── Determine the effective start of the blackout window ───────────
            if tag == 'KickOff':
                kickoff_count[gs] += 1
                if not first_kickoff_seen[gs]:
                    # First KickOff of this half: extend blackout backward to
                    # frame 0 to cover the gap between tracking start and kick.
                    blackout_start = 0
                    first_kickoff_seen[gs] = True
                    print(f"  [blackout] {gs} KickOff #1 (début de mi-temps): "
                          f"event_frame={event_frame} "
                          f"(tracking gap={event_frame/framerate:.1f}s) "
                          f"→ blackout étendu au frame 0")
                else:
                    # KickOff after a goal — standard window
                    blackout_start = event_frame
                    print(f"  [blackout] {gs} KickOff #{kickoff_count[gs]} (après but): "
                          f"event_frame={event_frame} "
                          f"({frame_to_minute(event_frame)}) "
                          f"→ blackout [{blackout_start}, {event_frame + offset_frames})")
            else:
                blackout_start = event_frame

            # Mark every frame in [blackout_start, event_frame + offset_frames)
            blackout[gs].update(range(blackout_start, event_frame + offset_frames))
            n_windows += 1
            break  # one setpiece tag per event is enough

    print(f"[OK] setpiece blackout — {n_windows} windows construits "
          f"(dont {kickoff_count['firstHalf']} KickOff en H1, "
          f"{kickoff_count['secondHalf']} KickOff en H2) | "
          f"{len(blackout['firstHalf'])} frames masqués H1, "
          f"{len(blackout['secondHalf'])} frames masqués H2.")
    return blackout
...
# ============================================================================
# Kickoff frame extractor (for accurate video-aligned timecodes)
# ============================================================================

def get_kickoff_frames(path_events, path_info, framerate=25, second_half_start_utc=None):
    """
    Return the tracking frame number of the actual kickoff for each half.

    Why this is needed
    ------------------
    DFL tracking starts BEFORE the ball is kicked. Frame 0 of each half does NOT
    correspond to 0:00 / 45:00 on the broadcast video.

    For the first half the gap is typically 0 s, but for the second half the H2
    tracking starts right after H1 ends (no file break), so there are roughly
    15-20 minutes of break at the beginning of secondHalf data.
    Using a fixed +67500 frame offset therefore drifts by ~18 minutes.

    This function anchors the video clock to the real kickoff event:
        video_seconds = (tracking_frame - kickoff_frame) / fps + half_start_seconds
    ...
    Returns
    -------
    dict { 'firstHalf': int, 'secondHalf': int }
    """
    info_root   = ET.parse(path_info).getroot()
    kickoff_str = info_root.find('.//General').attrib['KickoffTime']
    kickoff_utc = pd.Timestamp(kickoff_str).tz_convert('UTC')
    ...
    for event in ev_root.findall('Event'):
        ...
        if kickoff_frames['firstHalf'] is not None and kickoff_frames['secondHalf'] is not None:
            break

    if kickoff_frames['firstHalf'] is None:
        kickoff_frames['firstHalf'] = 0
        print("[WARN] firstHalf KickOff not found — timecodes may be off.")
    if kickoff_frames['secondHalf'] is None:
        kickoff_frames['secondHalf'] = 67500
        print("[WARN] secondHalf KickOff not found — falling back to 67500 frames.")

    print(
        f"[OK] kickoff frames — "
        f"H1: frame {kickoff_frames['firstHalf']} ({kickoff_frames['firstHalf']/framerate:.1f}s), "
        f"H2: frame {kickoff_frames['secondHalf']} ({kickoff_frames['secondHalf']/framerate:.1f}s into H2 tracking)."
    )
...
def process_match(
    ...
    """
    Full processing pipeline for a single match WITH SETPIECE OFFSETS:
    loading → filtering → discrete movements → distances → playing direction.

    **MODIFIED VERSION**: Movements whose start_frame falls within the offset
    window following a setpiece are excluded from the output:
    - ThrowIn      : +2 seconds
    - FreeKick     : +3 seconds
    - GoalKick     : +3 seconds
    - KickOff      : +4 seconds
    - CornerKick   : +4 seconds
    ...
    butterworth_order   : int   — filter order
    min_valley_distance : int   — minimum number of frames between valleys
    ...
    # Compute H2 anchor once here — avoids re-parsing XMLs 3x (once per function)
    second_half_start_utc = get_second_half_start_utc(path_events, path_info)
    ...
    # ------------------------------------------------------------------
    # 4b. Extract kickoff frames for video-aligned timecodes
    # ------------------------------------------------------------------
    # get_kickoff_frames() reads the EventTime of the first KickOff in each
    # half from the events XML and converts it to a tracking frame number.
    # This anchors the video clock correctly:
    #   H1: video_seconds = (frame - h1_kickoff_frame) / fps
    #   H2: video_seconds = (frame - h2_kickoff_frame) / fps + 45*60
    kickoff_frames_dict = get_kickoff_frames(
        path_events, path_info, framerate=framerate,
        second_half_start_utc=second_half_start_utc,
    )
    h1_kickoff = kickoff_frames_dict['firstHalf']
    h2_kickoff = kickoff_frames_dict['secondHalf']
    ...
    for team in ['Home', 'Away']:
        ...
        for half in ['firstHalf', 'secondHalf']:
            ...
            # ── Video-aligned timecodes ───────────────────────────────────────
            # Anchor: kickoff frame = 0:00 (H1) or 45:00 (H2) on the broadcast.
            # H1: video_s = (frame - h1_kickoff) / fps
            # H2: video_s = (frame - h2_kickoff) / fps + 45*60
            # This correctly handles:
            #   - pre-kick tracking frames (frames before kickoff → negative,
            #     clamped below to 0:00 for safety but excluded by blackout)
            #   - first-half stoppage time (naturally carried through)
            #   - the 15-20 min break stored as extra frames in secondHalf data
            H2_BASE_SECONDS = 45 * 60  # 45:00
            ...
            summary_df['start_timecode'] = summary_df.apply(
                lambda row: frame_to_video_timecode(row['start_frame'], row['half']),
                axis=1,
            )
            summary_df['end_timecode'] = summary_df.apply(
                lambda row: frame_to_video_timecode(row['end_frame'], row['half']),
                axis=1,
            )
    ...
    # ------------------------------------------------------------------
    # 9. Exclude movements whose START and/or END falls within a setpiece
    #    offset window.
    #    Movements that merely cross a blackout window mid-course (neither
    #    starting nor ending inside it) are intentionally kept.
    # ------------------------------------------------------------------
    ...
    def is_in_blackout(row):
        ...
        # Exclude if the START or the END of the movement is inside a blackout window
        return (start in bset) #or (end in bset)
    ...
    print(f"[OK] {n_excluded} movements excluded "
          f"(start or end falls within a setpiece offset window).")

    # ── Diagnostic: show earliest surviving movement per half ─────────────────
    for half in ['firstHalf', 'secondHalf']:
        sub = df_movements[df_movements['half'] == half]
        if not sub.empty:
            min_frame = sub['start_frame'].min()
            min_tc    = sub.loc[sub['start_frame'].idxmin(), 'start_timecode']
            print(f"    earliest surviving movement in {half}: "
                  f"frame {min_frame} ({min_tc})")

    print(f"[OK] {match_id} — {len(df_movements)} discrete movements extracted "
          f"(after setpiece blackout exclusion).")
